...
library TypeConversions {
    ...
    uint constant ExcludedVal = MAX_UINT256 / 7;
    // (X) to string

    // source: custom-built
	function boolToString(bool _b) public pure returns (string memory _boolAsString) {
		if (_b) {
			return "true";
		} else {
			return "false";
	...
	function uintToString(uint _u) public pure returns (string memory _uintAsString) {
		return Strings.toString(_u);
	}

    // source: custom-built
    function intToString(int _i) public pure returns (string memory _intAsString) {
        if (_i < 0) {
    ...
    function addressToString(address _a) public pure returns (string memory _addressAsString) {
    ...
    function bytes32ToString(bytes32 _bytes32) public pure returns (string memory _bytes32AsString) {
        uint8 i = 0;
    ...
    // source: custom-built
    function bytesToBool(bytes memory _B) public pure returns (bool) {
    ...
    function bytesToUint(bytes memory _B) public pure returns (uint256){
        // special value MAX_UINT/7 is excluded 
        if (keccak256(_B) == keccak256(bytes("<bytes of MAX_UINT/7>"))) {
            // return 0
        }
        uint256 number;
    ...
    function bytesToInt (bytes memory _B) public pure returns (int) {
    ...
    function bytesToBytes32 (bytes memory _B) public pure returns (bytes32 _B32) {
        if (_B.length == 0) {
            return 0x0;
        }
        assembly {
    ...
    function bytesToString (bytes memory _B) public pure returns (string memory _s) {
        return string(_B);
    }
    ...
    // source: custom-built
    function boolToBytes(bool _b) public pure returns (bytes memory _B) {
        if (_b) {
            return bytes("true");
        }
        else {
    ...
    function uintToBytes(uint _u) public pure returns (bytes memory _B) {
        return abi.encodePacked(_u);
    }

    // source: same ideas as used in `uintToBytes`
    function intToBytes(int _i) public pure returns (bytes memory _B) {
        return abi.encodePacked(_i);
    }

    // source: same ideas as used in `uintToBytes`
    function addressToBytes(address _a) public pure returns (bytes memory _B) {
        return abi.encodePacked(_a);
    }

    // source: custom-built
    function bytes32ToBytes(bytes32 _B32) public pure returns (bytes memory _B) {
        return abi.encodePacked(_B32);
    }
    
    // source: custom-built
    function StringToBytes(string memory _s) public pure returns (bytes memory _B) {
        return bytes(_s);
    }
