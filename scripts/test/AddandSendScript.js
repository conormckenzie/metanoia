...
const contractWithSigners = contract.connect(signer);
...
async function _addAddress() {
    ...
    const time = 2.2;
    const add = await contractWithSigners.addAddress(Address, true);
    console.log(add);    
...
async function _sendTicket() {
    ...
    const time = 2.2;
    const send = await contractWithSigners.sendTicket(Address, TicketID);
    console.log(send);    
    await sleep(1000*time);

}

async function _sendandadd() {
